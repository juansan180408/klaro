...
const App = (() => {
  ...
  const bindEvents = () => {
    ...
    // ── DASHBOARD ──
    $('btn-sesion-libre').addEventListener('click', () => iniciarSesion(null));

    $('areas-grid').addEventListener('click', e => {
      const card = e.target.closest('.area-card');
      if (card) iniciarSesion(card.dataset.area);
    });
    ...
    // ── RESUMEN ──
    $('btn-repetir').addEventListener('click', ()    => iniciarSesion(areaActiva));
  ...
  const iniciarSesion = async (area) => {
    areaActiva      = area;
    contadorSesion  = 0;
    preguntasSesion = [];
    ir('screen-loading');
  ...
  const siguientePregunta = () => {
    ...
    preguntaActual = Engine.seleccionarPregunta(state, sesionPreguntas, areaActiva);
...
document.addEventListener('DOMContentLoaded', App.init);
  ...
  const siguientePregunta = () => {
    ...
    preguntaActual = Engine.seleccionarPregunta(state, CONTENT.preguntas, areaActiva);
